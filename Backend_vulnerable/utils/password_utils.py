...
def verify_password(provided_password: str, stored_salt: str, stored_hash: str) -> bool:
    ...
    computed_hash = hashlib.pbkdf2_hmac(
        hash_algorithm,  # האלגוריתם
        provided_password.encode('utf-8'),  # הסיסמה שסופקה
        salt,  # ה-Salt המאוחסן
        iterations  # מספר החזרות
    )
...
def update_password_history(user_id: str, new_password: str):
    ...
    try:
        ...
        # שמירת ההיסטוריה המעודכנת במסד הנתונים
        query_update = f"""
        UPDATE users
