...
DATA_PATH = "../data/XSS_requests.json"
...
def test_xss():
    ...
    endpoints = [
        ...
        ("login", "POST", "/users/login", test_requests["attacks"]["login"], None, 400),
        ("password reset", "POST", "/users/password-reset", test_requests["attacks"]["password_reset"], None, 400),
        ("update user", "PUT", f"/users/{test_requests['attacks']['update_user']['user_id']}",
         test_requests["attacks"]["update_user"], {"Authorization": f"Bearer {token}"}, 400),
    ]

    for name, method, endpoint, payload, headers, expected_status in endpoints:
        try:
            print(f"[Attack] Testing XSS on {name} endpoint...")
            if method == "POST":
            ...
            errors.append(f"Error in XSS test on {name}: {e}")
